...
PHASE="${PHASE:-all}"
OUT_ROOT="${OUT_ROOT:-artifacts/attnres_engram_multiseed}"
LIMIT="${LIMIT:-512}"
...
train_seed() {
  ...
  run_container "${gpu}" -m eval.transformer.train_text_lm_compare \
    --device cuda \
    --variants attnres engram_noconv_attnres \
    --seed "${seed}" \
    --seq-len 256 \
    --batch-size 8 \
    --train-steps 400 \
...
eval_seed() {
  ...
  run_container "${gpu}" -m eval.transformer.paper_tasks_compare \
    ...
    --engram-ckpt "${OUT_ROOT}/train/attnres_seed${seed}/model.pt" \
    --limit "${LIMIT}" \
    --max-len "${MAX_LEN}" \
    --out "${OUT_ROOT}/eval/attnres_vs_baseline_seed${seed}.json"
  run_container "${gpu}" -m eval.transformer.paper_tasks_compare \
    --device cuda \
    --baseline-ckpt "/historical-artifacts/text_lm_compare_det/engram_noconv_seed${seed}/model.pt" \
    --engram-ckpt "${OUT_ROOT}/train/engram_noconv_attnres_seed${seed}/model.pt" \
    --limit "${LIMIT}" \
    --max-len "${MAX_LEN}" \
    --out "${OUT_ROOT}/eval/combined_vs_engram_seed${seed}.json"
}
