...
PHASE="${PHASE:-all}"
OUT_ROOT="${OUT_ROOT:-artifacts/attnres_engram_v1_multiseed}"
LIMIT="${LIMIT:-512}"
...
train_seed() {
  ...
  run_container "${gpu}" -m eval.transformer.train_text_lm_compare \
    --device cuda \
    --variants engram_noconv_attnres_v1 \
    --seed "${seed}" \
    --seq-len 256 \
    --batch-size 8 \
    --train-steps 400 \
...
eval_seed() {
  ...
  run_container "${gpu}" -m eval.transformer.paper_tasks_compare \
    ...
    --engram-ckpt "${OUT_ROOT}/train/engram_noconv_attnres_v1_seed${seed}/model.pt" \
    --limit "${LIMIT}" \
    --max-len "${MAX_LEN}" \
    --out "${OUT_ROOT}/eval/v1_vs_engram_seed${seed}.json"
}
...
echo "[$(date --iso-8601=seconds)] v1 campaign complete phase=${PHASE}"
